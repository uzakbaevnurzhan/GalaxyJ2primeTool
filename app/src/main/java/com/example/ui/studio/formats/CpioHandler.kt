...
object CpioHandler {
    fun unpack(cpioFile: File, outputDir: File): Boolean {
        // Simple CPIO newc parser implementation
        ...
        try {
            RandomAccessFile(cpioFile, "r").use { raf ->
                while (raf.filePointer < raf.length()) {
                    ...
                    raf.read(nameBytes)
                    val name = String(nameBytes, 0, namesize.toInt() - 1) // exclude null terminator
                    ...
                    when (type) {
                        ...
                        8L -> { // regular file
                            ...
                            val dataBytes = ByteArray(filesize.toInt())
                            raf.read(dataBytes)
                            outPath.writeBytes(dataBytes)
                        }
                        10L -> { // symlink
                            outPath.parentFile?.mkdirs()
                            val targetBytes = ByteArray(filesize.toInt())
                            raf.read(targetBytes)
                            // We just save symlink as a text file for now, real symlink in android is tricky without root
                            outPath.writeText("SYMLINK:" + String(targetBytes, 0, filesize.toInt()))
                        }
